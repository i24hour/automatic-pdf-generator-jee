...
\usepackage{booktabs}  % For Matrix Match tables
\usepackage{array}     % For table column formatting
\usepackage[framemethod=tikz]{mdframed}  % For Paragraph passage box
...
\newcommand{\fourchoices}[4]{%
    \vspace{2pt}%
    \par\noindent\hspace{0.5cm}(a) #1%
    \par\noindent\hspace{0.5cm}(b) #2%
    \par\noindent\hspace{0.5cm}(c) #3%
    \par\noindent\hspace{0.5cm}(d) #4%
    \vspace{4pt}%
}
...
\begin{enumerate}[label=\textbf{Q.\arabic*}, itemsep=10pt]
    ...
    \BLOCK{ for q in questions }
        ...
        \item\relax \VAR{q.text}
        
        % Check if new PNG diagram (from LLM SVG) exists
        \BLOCK{ if q.diagram_png_path }
        \begin{center}
            \includegraphics[width=0.55\textwidth]{\VAR{q.diagram_png_path}}
        \end{center}
        \BLOCK{ endif }

        % Check if legacy TikZ diagram exists
        ...
        \BLOCK{ elif q.type == 'long_answer' }
            ...
            \vspace{0.2cm}
            \par\noindent\textcolor{gray}{\dotfill}
        \BLOCK{ elif q.type == 'matrix_match' }
            \hfill \textbf{[8 Marks]}
            \vspace{0.3cm}
            \begin{center}
            ...
            \hline
            \BLOCK{ for i in range([q.list1|length, q.list2|length]|min) }
            \VAR{q.list1[i]} & \VAR{q.list2[i]} \\
            \BLOCK{ endfor }
            \BLOCK{ if q.list1|length > q.list2|length }
            \BLOCK{ for i in range(q.list2|length, q.list1|length) }
            \VAR{q.list1[i]} & \\
            \BLOCK{ endfor }
            \BLOCK{ elif q.list2|length > q.list1|length }
            \BLOCK{ for i in range(q.list1|length, q.list2|length) }
             & \VAR{q.list2[i]} \\
            \BLOCK{ endfor }
            \BLOCK{ endif }
            \hline
            ...
            \end{center}
            \vspace{0.2cm}
        \BLOCK{ elif q.type == 'paragraph' }
            \hfill \textbf{[Comprehension]}
            \vspace{0.2cm}
            \begin{mdframed}[backgroundcolor=gray!10, linecolor=gray!60, linewidth=0.8pt, innerleftmargin=8pt, innerrightmargin=8pt, innertopmargin=6pt, innerbottommargin=6pt]
            \small\textit{\VAR{q.passage|default('')}}
            \end{mdframed}
            \vspace{0.3cm}
            \BLOCK{ if q.sub_questions }
            \BLOCK{ for sq in q.sub_questions }
            \par\noindent (\VAR{loop.index|string}) \VAR{sq.text}
            \BLOCK{ if sq.options }
...
% Wrap answer texts to prevent long sentences from pushing columns off the 21cm A4 page.
\begin{tabular}{|c|>{\raggedright\arraybackslash}p{0.22\textwidth}||c|>{\raggedright\arraybackslash}p{0.22\textwidth}||c|>{\raggedright\arraybackslash}p{0.22\textwidth}|}
\hline
...
\BLOCK{ endfor }
\BLOCK{ if not (questions|length is divisibleby 3) }
    % Pad remaining columns if questions don't perfectly fill 3-column rows
    \BLOCK{ if (questions|length % 3) == 1 } & & & \\ \hline \BLOCK{ endif }
    \BLOCK{ if (questions|length % 3) == 2 } & \\ \hline \BLOCK{ endif }
\BLOCK{ endif }
\end{tabular}
\end{table}
\BLOCK{ endif }
...
\VAR{q.solution}
\BLOCK{ else }
    \BLOCK{ if q.type == 'mcq' or q.type == 'mcq_multi' }
    The correct option is \textbf{\VAR{q.answer}}.
    \BLOCK{ else }
    \textit{Solution not provided.}
    \BLOCK{ endif }
\BLOCK{ endif }
}
