...
# ---------------------------------------------------------------------------
# Cashfree config  (set these in Cloud Run env / .env)
# ---------------------------------------------------------------------------
...
# Plan limits used by rate-limit logic (also imported in main.py)
PLAN_LIMITS = {
    "free":     {"pdf": 5,         "test": 4},
    "earth":    {"pdf": 10,        "test": 999999},   # unlimited tests
    "universe": {"pdf": 999999,    "test": 999999},   # unlimited all
}
...
class CreateOrderResponse(BaseModel):
    ...
    amount: float
    plan_name: str
    cf_env: str   # "sandbox" | "production"  – so frontend knows which SDK mode to use
...
async def create_order(
    ...
    payload = {
        "order_id": order_id,
        "order_amount": plan["amount_paise"] / 100,   # Cashfree needs ₹ not paise
        "order_currency": "INR",
        "customer_details": {
...
async def cashfree_webhook(request: Request, db: Session = Depends(get_db)):
    ...
    We verify the HMAC-SHA256 signature before trusting the payload.
    """
...
def _activate_subscription(order: PaymentOrder, db: Session):
    ...
    # Upsert UserSubscription (one row per user)
